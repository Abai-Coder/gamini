...
export const search = async (query, count = 1) => {
  ...
  const result = store.similaritySearch(query, count)
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are an AI assistant, based on user preference from query provide answer on which movie to watch',

      },
      {
        role: 'user',
        content: `User was looking for a movie based on this query: ${query}, so provide answer with suggestion of this movie: ${result}`,
      }
    ]
  })
...
console.log(await search('посоветуй добрый фильм на ночь и ответь на русском'))
